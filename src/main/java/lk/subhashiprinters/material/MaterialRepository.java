...
//     List<Material> list();
  ...
  @Query("select new Material(m.id, m.name, m.code,m.unit_price,m.width,m.height) from Material m where m.material_status_id.id=1 and m.id in " +
     ...
     @Query("select new Material(m.id, m.name, m.code,m.unit_price,m.width,m.height,m.material_unit_type_id) from Material m where m.material_status_id.id=1 and m.paper_ink_type_id.id=?1")
