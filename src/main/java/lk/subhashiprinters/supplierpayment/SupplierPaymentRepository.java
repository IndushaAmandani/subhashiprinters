...
public interface SupplierPaymentRepository extends JpaRepository<SupplierPayment, Integer> {
    ...
    @Query("select new SupplierPayment(sp.id,sp.bill_no,sp.total_amount,sp.paid_amount,sp.balance_amount,sp.added_date,sp.supplier_id,sp.material_recieve_note_id,sp.supplier_payment_type_id,sp.supplier_payment_status_id) from SupplierPayment sp order by sp.id desc ")
