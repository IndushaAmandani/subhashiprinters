...
public interface CustomerRepository extends JpaRepository<Customer, Integer> {
    ...
    @Query(value = "select C from Customer C where C.id not in (select co.customer_id from CustomerOrder co where co.order_status_id.id<>5 and co.order_status_id.id<>6)")
    List<Customer> getListOfDeletableCustomers();
