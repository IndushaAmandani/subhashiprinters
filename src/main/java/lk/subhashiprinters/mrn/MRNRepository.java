...
//    @Query(value = "SELECT concat('SUP',lpad(substring(max(s.reg_no),4)+1 , 7 ,'0')) FROM subhashiprinters.supplier as s;",nativeQuery = true)
    @Query(value = "SELECT concat('MRN',lpad(substring(max(mrno.recieve_no),4)+1,6,'0')) FROM subhashiprinters.material_recieve_note as mrno;",nativeQuery = true)
    String getNextMRNno();

    @Query(value = "select new MRN(mrno.id,mrno.recieve_no,mrno.recieve_date,mrno.net_amount,mrno.purchase_order_id) from MRN  mrno where mrno.material_recieve_note_status_id.id = 1")
     List <MRN> getnotpaid();

    @Query(value = "select new MRN(mrno.id,mrno.recieve_no,mrno.recieve_date,mrno.net_amount,mrno.purchase_order_id) from  MRN mrno where mrno.material_recieve_note_status_id.id=1 and mrno.purchase_order_id.supplier_id.id=?1 and  mrno.purchase_order_id.supplier_id.supplier_status_id.id=1")
