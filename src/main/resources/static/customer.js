...
const refreshCustomerTable = () => {
    ...
    let dispalyPropertyList = [ 'customer_code', 'name', 'mobile', 'customer_email', 'customer_category_id.name','customer_type_id.name','customerstatus_id.name'];
    //Property type list
...
const formRefill = (ob, rowno) => {
    ...
    $.ajax('/customer/getbyid?id='+ob.id,{
        async: false,
        dataType:'json',
        success: function (data,status, xhr){
            oldcustomer = data;
